...
really just control flow: \emph{if A then B else C} (\(A\:?\:B\::\:C\)). We
want to have some condition that chooses between two expressions. We know how
to do that already (see section \ref{combinatorylogic}). \(K\) choses the first
of two expressions and \(KI\) the latter. We can define true to be \(K\) and
false to be \(KI\):
...
If you were to write this in pure lambda calculus (using \(NOT:=C\), because it
is simpeler to write out), you would get the following:
\small
\begin{align}
	&\enspace\lambda xy.
...
only gets selected if \(xyx=F\). There are two ways for \(xyx=F\) being true;
